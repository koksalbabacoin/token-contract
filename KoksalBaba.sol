...
contract KoksalBaba is Context, IBEP20, Ownable {
    ...
    address public marketingWallet = 0xa0Effb7E2963ABE9FD6B3B97237Bde85a02A4E90;
    address public KoksalBribeWallet = 0xCfCc3E696C0d792219477f80926D34BA8A537601;

    string private constant _name = "Koksal Baba";
    string private constant _symbol = "KOKSAL";
    ...
    Taxes public taxes = Taxes(2, 5, 0, 1);
    ...
    struct valuesFromGetValues {
        ...
        uint256 rMarketing;
        uint256 rLiquidity;
        uint256 rKoksalBribe;
        uint256 tTransferAmount;
        uint256 tRfi;
        uint256 tMarketing;
        uint256 tLiquidity;
        uint256 tKoksalBribe;
    }
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[KoksalBribeWallet] = true;
    ...
    function setTaxes(
        ...
        uint256 _marketing,
        uint256 _liquidity,
        uint256 _KoksalBribe
    ) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _KoksalBribe ) <= 12, "Must keep fees at 12% or less");
        taxes = Taxes(_rfi, _marketing, _liquidity, _KoksalBribe);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _KoksalBribe ) <= 12, "Must keep fees at 12% or less");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _KoksalBribe);
        emit FeesChanged();
    }
    ...
    function _takeKoksalBribe(uint256 rKoksalBribe, uint256 tKoksalBribe) private {
        totFeesPaid.KoksalBribe += tKoksalBribe;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tKoksalBribe;
        }
        _rOwned[address(this)] += rKoksalBribe;
    }
    ...
    function _getValues(
        ...
        (to_return.rKoksalBribe) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
    ...
    function _getTValues(
        ...
        s.tKoksalBribe = (tAmount * temp.KoksalBribe) / 100;
        s.tTransferAmount =
            tAmount -
            s.tRfi -
            s.tMarketing -
        ...
        )
    {
        ...
        uint256 rKoksalBribe = s.tKoksalBribe * currentRate;
        rTransferAmount =
            rAmount -
            rRfi -
            rMarketing -
        ...
        )
    {
        ...
        rKoksalBribe = s.tKoksalBribe * currentRate;
        return (rKoksalBribe);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tKoksalBribe
            );
        }
        if (s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if (s.rKoksalBribe > 0 || s.tKoksalBribe > 0) _takeKoksalBribe(s.rKoksalBribe, s.tKoksalBribe);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        uint256 KoksalBribeAmt = unitBalance * 2 * temp.KoksalBribe;
        if (KoksalBribeAmt > 0) {
            payable(KoksalBribeWallet).sendValue(KoksalBribeAmt);
        }
    }
    ...
    function updateKoksalBribeWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        KoksalBribeWallet = newWallet;
    }
